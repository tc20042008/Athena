        ...
        for (int group = 0; group < ThreadMap::Iterations::kGroup; ++group) {
          ...
          for (int row = 0; row < ThreadMap::Iterations::kRow; ++row) {
            ...
            for (int column = 0; column < ThreadMap::Iterations::kColumn; ++column) {
              ...
              output_frag_ptr[frag_offset] = output_op(compute_frag_ptr[frag_offset], row_offset, column_offset);
            }
          }
        }
      }
  ...
  struct SourceAspectNeeded
  {
      ...
      typename OutputTileIterator::Fragment const &source_fragment)
    {
      ...
      typename OutputTileIterator::Element const *source_ptr =
        reinterpret_cast<typename OutputTileIterator::Element const *>(&source_fragment);
        ...
        for (int group = 0; group < ThreadMap::Iterations::kGroup; ++group) {
          ...
          for (int row = 0; row < ThreadMap::Iterations::kRow; ++row) {
            ...
            for (int column = 0; column < ThreadMap::Iterations::kColumn; ++column) {
              ...
              output_frag_ptr[frag_offset] = output_op(compute_frag_ptr[frag_offset], source_frag_ptr[frag_offset], row_offset, column_offset);
            }
          }
        }
      }
    ...
    void apply_output_operator(
      ...
      typename SharedLoadIterator::Fragment const &aligned_accum_fragment)
    {
      apply_output_operator(output_iterator, output_fragment, output_op, aligned_accum_fragment, source_fragment);
    }
  };
...
public:
    ...
    SourceAspect source)
  {
    ...
    for (int iter = 0; iter < OutputTileIterator::kIterations; ++iter)
    {
      ...
      typename OutputTileIterator::Fragment output_fragment;
      source.apply_output_operator(destination_iterator, output_fragment, output_op, aligned_accum_fragment[0]);
